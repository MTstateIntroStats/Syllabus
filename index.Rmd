...
* [STAT 216 calendar](calendars/F24-Stat216_Calendar.pdf)
...
* Per the [STAT 216 calendar](calendars/F24-Stat216_Calendar.pdf), the course will be broken down into topics, modules, and units.
* To begin each topic, students will 
  + **read** assigned sections of the online textbook on that topic's content and complete (optionally) the reading guides in the Stat 216 coursepack.
  + **watch** assigned videos on D2L and complete (optionally) the video guides located in the Stat 216 coursepack
  + complete **video/reading** quizzes on [Gradescope](https://www.gradescope.com/) **before** their class meets.
* Instructor-lead **Guided Activities** will take place during class, with students able to test themselves on the topic during **Activity** days.
* After all activities within a module have been completed, students will complete a **Module Lab**, which will be completed and turned into Gradescope during class.
...
grades$category <- fct_relevel(grades$category, "Video/Reading Quizzes", "Activities", "Module Labs" , "Assignments", "Midterm exams (2)", "Final exam")
grades$location <- rep(1, 6)
grades %>% ggplot(aes(x=location, y=percent)) +
  geom_col(aes(fill=category), col='black') + 
  guides(fill=FALSE) + 
  geom_text(aes(x=rep(1, 6), 
                label=c("Video/Reading Quizzes (5%)",
                        "Activities (10%)",
                        "Module Labs (10%)",
                        "Assignments (15%)",
                        "Midterm exams (30%)",
                        "Final exam (30%)"), 
                y=c(97.5,90,80,67.5,45,15))) + 
...
* Midterm exams are closed book, closed notes. 
* A formula sheet will be provided to use during the exam (also released with the potential midterm exam questions).
* You will be allowed a calculator on the midterm exams. 
* You will **not** be required to use Rstudio on the midterm exams.
...
Individual common hour final exam will be **TBD (set by the Registrar, be sure to check the Common Hour Final listings!)**. 
...
* **Video/Reading Quizzes**: 
    - You may take the video/reading quizzes in Gradescope as many times as you like up until the due date using the Resubmit button to re-open a quiz. 
...
* **Activities, Labs, and Assignments**: Attendance in this course is critical for success and is therefore required. The in-class activity and lab grades are a proxy for attendance and engagement. Students are expected to be in class during in-class activities and labs to provide support to each other and their teammates while working through the material. We will not record or post lectures/asynchronous learning opportunities. Students get a “free pass” for up to three activities per Unit, no questions asked, but are requested to communicate with their section instructor anytime they miss class. Illnesses/emergencies/school-related absences are included in these three; if students have extraneous circumstances, they are encouraged to talk to their instructor. 
    ...
    - For labs and assignments, while the due dates are 9 pm on the day the lab is completed in class and Mondays, respectively, you will be allowed to turn in work until 11:59 pm on the due date.  Lab and assignment submissions that are received between 9 pm and 11:59 pm will receive a 5% grade deduction.  

* **Exams**: 
    - Students that are in quarantine but healthy enough to take the exam should email Student Success Coordinator [Jade Schmidt](mailto: jade.schmidt2@montana.edu) to make alternative arrangements.  The group final exam may be taken remotely and proctored via WebEx but all individual exams must be taken in person.
...
Please see the How to cite ChatGPT in MLA Style [resource](https://style.mla.org/citing-generative-ai/). I encourage you to use AI language models to enhance your writing and coding skills, experiment with its capabilities, and learn from its suggestions. If you have any questions or concerns regarding using AI language models for assignments, please discuss them with us.
